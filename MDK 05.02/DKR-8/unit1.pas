...
begin
 ...
 '-':a:=round(a-b);
 '*':a:=round(a*b);
 'd':a:=round(a) div round(b);
 'm':a:=round(a) div round(b)
 end;
 b:=0;
 output.text:='';
 operation:='d';
...
begin
  ...
  begin
  b := StrToFloat(output.text);
  if (b = 0) and ((operation = 'd') or (operation = 'm')) then
  begin
  output.text := 'Невозможно';
  Exit;
  end
...
begin
b:=strtofloat(output.text);
if ((operation='d') or (operation ='m')) and (b=0) then
begin
output.text:='Невозможно';
Exit;
end;
...
begin
  ...
  begin
  b:=strtofloat(output.text);
  if ((operation='m') or (operation='d')) and (b=0) then
  begin
  output.text:='Невозможно';
  Exit;
  end;
...
begin
  ...
  output.text:=''
    ...
    begin
    a:=round(sqrt(a));
    output.text:=floattostr(round(a));
    end;
    end;
end;
...
begin
  ...
  begin
  b:=strtofloat(output.text);
  if (b=0) and ((operation='m')or (operation='d')) then
    begin
    output.text :='Невозможно';
    exit;
    end;
